...
function belong_list_events_for_user() {
    ...
    foreach ($event_posts as $post) {
        ...
        if ($assignment_client['ID'] == $current_user->ID && $assignment_type == 'Events') {
            ...
            ?><h2><a href="<?php get_permalink($post->ID); ?>"><?php echo $post->post_title.' '.$assignment_select_event; ?></a></h2><?php
        }
    }
    return ob_get_clean();
}
...
function belong_list_modules_for_user() {
    ...
    foreach ($module_posts as $post) {
        ...
        if ($assignment_client['ID'] == $current_user->ID) {
            ...
            ?><h2><a href="<?php get_post_permalink($post->ID); ?>"><?php echo $post->post_title; ?></a></h2><?php 
        }
    }
    return ob_get_clean();
}
