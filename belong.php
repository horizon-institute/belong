...
function client_registration_form() {
    ...
    $cm = unserialize($cm);
    // $client_meta = get_post_meta($post_id, "client_profile_" . $id);
    // foreach($client_meta as $cm) { 
    //     $cm = unserialize($cm); 
    // }
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        if(isset($_POST)) {
            ...
            if (! add_post_meta($post_id, "client_profile_" . $id, serialize($_POST), true)) {
                update_post_meta($post_id, "client_profile_" . $id, serialize($_POST));
            }
...
function text_field($name, $title, $col, $cm) {
    ...
    echo '<input class="form-control" type="text" name="' . $name . '" pattern="[a-zA-Z0-9 ]+" value="' . $cm[$name][0]  . '" size="40" />';
    echo '</div>';
}
