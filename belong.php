    ...
    if ($assignment_posts) {
        ...
        foreach ($assignment_posts as $post) {
            ...
            if (belong_is_current_user_selected($assignment_client, $current_user->ID) && $assignment_type == 'Events') {
                $assignment_event = get_field('assignmefunction belong_list_modules_for_user() {
    ob_start();
    $counter = 0;
    $current_user = belong_get_user_info();
    $assignment_args = array(
...
add_shortcode('user_modules', 'belong_list_modules_for_user');nt_select_event', $post->ID);
