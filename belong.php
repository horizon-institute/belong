...
function children($cm) {
    ...
    echo '<button class="btn btn-success btn-sm" type="button"  onclick="child_fields(' . json_encode($cm) . ');"> <span class="glyphicon glyphicon-plus" aria-hidden="true"></span></button>';
      ...
      <?php
        ...
        if(isset($child_names)) {
            ...
            for($i=0; $i < $count; $i++) { ?>
                <script type="text/javascript">
                    jQuery(document).ready(function() {
                        child_fields(<?php echo json_encode($cm); ?>);
                    });
                </script>
                <?php
            }
