...
function personal_form($id, $client_profile)
{
    ...
    user_field("pw-email", "EMAIL ADDRESS", "user_email", $id, "6", $client_profile, 'email');
    echo '</fieldset>';
    echo '</div>';
    echo '<div class="row">';
    select_field(array("Male", "Female"), "pw-client-gender", "GENDER", "6", $client_profile);
    text_field("pw-client-telephone", "TELEPHONE NUMBER", "6", $client_profile, 'tel');
    echo '</div>';
    echo '<div class="row">';
 ...
 * User text field
 ************************************************/
function user_field($name, $title, $field_name, $id, $col, $client_profile, $type = 'text')
{
    $user = belong_get_user_by("ID", $id);
    $field = $user->$field_name;
    echo '<div class="col-md-' . $col . '" >';
    echo '<label>' . $title . '</label>';
    echo '<input class="form-control" type="' . $type . '" name="' . $name . '" pattern="[a-zA-Z0-9 ]+" value="' . (isset($field) ? esc_attr($field) : '') . '" size="50" />';
    echo '</div>';
}
